 ...
 */
std::string SafeReadFile( const std::string& aFilePath, const std::string& aReadType );
...
protected:
    ...
    std::string    m_source;         ///< origin of text lines, e.g. filename or "clipboard"
};
...
public:
     ...
     */
    FILE_LINE_READER( const std::string& aFileName, unsigned aStartingLineNumber = 0,
     ...
     * @param aMaxLineLength is the number of bytes to use in the line buffer.
     */
    FILE_LINE_READER( FILE* aFile, const std::string& aFileName, bool doOwn = true,
...
public:
     ...
     */
    STRING_LINE_READER( const std::string& aString, const std::string& aSource );
...
public:
     ...
     */
    INPUTSTREAM_LINE_READER( std::ifstream* aStream, const std::string& aSource );

    char* ReadLine() override;

protected:
    std::ifstream* m_stream;        ///< The input stream to read.  No ownership of this pointer.
};
...
public:
     ...
     */
     ...
     std::string Quotew( const std::string& aWrapee ) const;
...
public:
     ...
     */
    FILE_OUTPUTFORMATTER( const std::string& aFileName, const char* aMode = "wt",
                          char aQuoteChar = '"' );
...
class PRETTIFIED_FILE_OUTPUTFORMATTER : public OUTPUTFORMATTER
{
public:
    PRETTIFIED_FILE_OUTPUTFORMATTER( const std::string& aFileName, const char* aMode = "wt",
                                     char aQuoteChar = '"' );
