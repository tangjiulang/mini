...
KIID::KIID()
{
    ...
    {
        spdlog::error( "A Boost UUID entropy exception was thrown in {}:{}.",
...
KIID::KIID( const QString& aString ) :
        KIID( std::string( aString.toStdString() ) )
{
}
...
{
    static QString niluuidStr = niluuid.AsString().data();

    if( aCandidate.size() != niluuidStr.size() )
        return false;
...
std::string KIID::AsString() const
{
    
    return boost::uuids::to_string(m_uuid).data();
}
...
std::string KIID::AsLegacyTimestampString() const
{
    return std::format( "{:08X}", (unsigned long) AsLegacyTimestamp() );
}
...
{
    QStringList parts = aString.split('/');
    for( const QString& pathStep : parts)
    {
        if( !pathStep.isEmpty() )
...
std::string KIID_PATH::AsString() const
{
    std::string path;
