...
void ANTIALIASING_SMAA::loadShaders()
{
    ...
    function->glGenTextures( 1, &smaaAreaTex );
    function->glBindTexture( GL_TEXTURE_2D, smaaAreaTex );
    function->glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE );
    function->glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE );
    ...
    function->glTexImage2D( GL_TEXTURE_2D, 0, GL_RG8, AREATEX_WIDTH, AREATEX_HEIGHT, 0, GL_RG,
                  GL_UNSIGNED_BYTE, areaTexBytes );
    checkGlError( "loading smaa area tex", __FILE__, __LINE__ );

    function->glGenTextures( 1, &smaaSearchTex );
    function->glBindTexture( GL_TEXTURE_2D, smaaSearchTex );
    function->glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE );
    function->glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE );
    function->glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR );
    function->glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
    function->glTexImage2D( GL_TEXTURE_2D, 0, GL_R8, SEARCHTEX_WIDTH, SEARCHTEX_HEIGHT, 0, GL_RED,
    ...
    edge_detect_shader = "glsl_smaa_pass_1_frag_luma";
...
void ANTIALIASING_SMAA::Present()
{
    ...
    function->glActiveTexture( GL_TEXTURE0 );
    function->glBindTexture( GL_TEXTURE_2D, edgesTex );
    function->glActiveTexture( GL_TEXTURE1 );
    function->glBindTexture( GL_TEXTURE_2D, smaaAreaTex );
    function->glActiveTexture( GL_TEXTURE3 );
    function->glBindTexture( GL_TEXTURE_2D, smaaSearchTex );
    ...
    function->glBindTexture( GL_TEXTURE_2D, sourceTexture );
    function->glActiveTexture( GL_TEXTURE1 );
    function->glBindTexture( GL_TEXTURE_2D, blendTex );
