...
#include <polygon_triangulation.hxx>
#include <seg.hxx>                    // for SEG, OPT_VECTOR2I
#include <shape.hxx>
#include <shape_line_chain.hxx>
#include <shape_poly_set.hxx>
#include <box2.hxx>                       // for BOX2I
#include <util.hxx>                       // for KiROUND, rescale
#include <vector2d.hxx>                   // for VECTOR2I, VECTOR2D, VECTOR2
#include <hash.hxx>
#include <mmh3_hash.hxx>
...
int SHAPE_POLY_SET::AddOutline( const SHAPE_LINE_CHAIN& aOutline )
{
    ...
    if (aOutline.IsClosed())
        poly.back().SetClosed(true);
    else
        spdlog::warn("Warning: non-closed outline added to SHAPE_POLY_SET");
                                ...
                                const SHAPE_POLY_SET& aOtherShape )
{
    ...
    {
        spdlog::error("Boolean ops on curved polygons are not supported. You should call "
    ...
    Clipper2Lib::ZCallback64 callback =
            ...
            {
                auto arcIndex =
                    [&]( const size_t& aZvalue, const size_t& aCompareVal = -1 ) -> size_t
                    {
                        size_t retval;
                ...
                auto arcSegment =
                    [&]( const size_t& aBottomZ, const size_t aTopZ ) -> size_t
                    {
                        size_t retval = arcIndex( aBottomZ );
                ...
                size_t e1ArcSegmentIndex = arcSegment( e1bot.z, e1top.z );
                size_t e2ArcSegmentIndex = arcSegment( e2bot.z, e2top.z );
    ...
    for( const Clipper2Lib::Path64& n : aPath )
    {
        ...
        else
        {
            if (!path.empty())
                ...
                spdlog::trace("Cannot add a hole before an outline");
        }
...
static void fractureSingleCacheFriendly( SHAPE_POLY_SET::POLYGON& paths )
{
    ...
    {
        spdlog::warn("Polygon has more points than int limit");
        return;
    }
    ...
    for( auto it = sorted_paths.begin() + 1; it != sorted_paths.end(); it++ )
    {
        ...
        if( !edge )
        {
            spdlog::warn("Broken polygon, dropping path" );
...
{
    if (!aOutlineOnly)
        spdlog::warn("Warning: SHAPE_POLY_SET::SquaredDistance does not yet support aOutlineOnly==true");
                                         ...
                                         std::vector<std::unique_ptr<TRIANGULATED_POLYGON>>* aHintData )
{
    ...
    if( aPartition )
    {
        for( int ii = 0; ii < OutlineCount(); ++ii )
        {
            ...
            {
                spdlog::trace( "{} Failed to triangulate partitioned polygon {}", TRIANGULATE_TRACE, ii );
            }
            else
            {
                m_hash = checksum();
    ...
    else
    {
        ...
        {
            spdlog::trace("Failed to triangulate polygon", TRIANGULATE_TRACE);
        }
        else
        {
            m_hash = checksum();
