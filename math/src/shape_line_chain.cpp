...
#include <kicad_algo.hxx> // for alg::run_on_pair
#include <circle.hxx>
#include <seg.hxx>    // for SEG, OPT_VECTOR2I
#include <shape_line_chain.hxx>
#include <shape_poly_set.hxx>
#include <box2.hxx>       // for BOX2I
#include <util.hxx>       // for rescale
#include <vector2d.hxx>   // for VECTOR2, VECTOR2I
#include <box2_minmax.hxx>
#include <trigo.hxx>           // for RotatePoint

class SHAPE;

const size_t                     SHAPE_LINE_CHAIN::SHAPE_IS_PT = -1;
const std::pair<size_t, size_t> SHAPE_LINE_CHAIN::SHAPES_ARE_PT = { SHAPE_IS_PT, SHAPE_IS_PT };
...
SHAPE_LINE_CHAIN::SHAPE_LINE_CHAIN( const Clipper2Lib::Path64&          aPath,
        ...
        m_width( 0 )
{
    std::map<size_t, size_t> loadedArcs;
    m_points.reserve( aPath.size() );
    m_shapes.reserve( aPath.size() );

    auto loadArc =
        [&]( size_t aArcIndex ) -> size_t
        {
                                                     ...
                                                     std::vector<SHAPE_ARC>& aArcBuffer ) const
{
    ...
    size_t          shape_offset = aArcBuffer.size();
...
void SHAPE_LINE_CHAIN::splitArc( size_t aPtIndex, bool aCoincident )
{
    if( aPtIndex < 0 )
    ...
    //wxCHECK_MSG( aPtIndex < static_cast<size_t>( m_shapes.size() ), /* void */,
    ...
    if( IsSharedPt( aPtIndex ) || IsArcEnd( aPtIndex ) )
    {
        ...
        size_t firstArcIndex = m_shapes[aPtIndex].first;
    ...
    size_t    currArcIdx = ArcIndex( aPtIndex );
...
int SHAPE_LINE_CHAIN::NextShape( int aPointIndex ) const
{
    ...
    if (m_shapes[aPointIndex].first != SHAPE_IS_PT)
        ...
        spdlog::trace("malformed chain!");

    size_t currentArcIdx = ArcIndex( aPointIndex );
...
void SHAPE_LINE_CHAIN::RemoveShape( int aPointIndex )
{
    ...
    if( !IsArcStart( start ) )
    {
        ...
        while( start > 0 && ArcIndex( static_cast<size_t>( start ) - 1 ) == arcIdx )
            start--;
    }
...
const SHAPE_LINE_CHAIN SHAPE_LINE_CHAIN::Slice( int aStartIndex, int aEndIndex, int aMaxError ) const
{
    ...
    if( IsArcSegment( aStartIndex ) && !IsArcStart( aStartIndex ) )
    {
        ...
        size_t          arcToSplitIndex = ArcIndex( aStartIndex );
    ...
    for( int i = aStartIndex; i <= aEndIndex && i < numPoints; i = NextShape( i ) )
    {
        ...
        {
            if (!IsArcSegment(i))
                spdlog::trace("Still on an arc segment, we missed something...");
...
void SHAPE_LINE_CHAIN::Append( const SHAPE_LINE_CHAIN& aOtherLine )
{
    ...
    for( int i = 1; i < aOtherLine.PointCount(); i++ )
    {
        ...
        size_t arcIndex = aOtherLine.ArcIndex( i );

        if( arcIndex != size_t( SHAPE_IS_PT ) )
        {
...
void SHAPE_LINE_CHAIN::Insert( size_t aVertex, const SHAPE_ARC& aArc, int aMaxError )
{
    ...
    //@todo need to check we aren't creating duplicate points at start or end
    std::vector<std::pair<size_t, size_t>> new_points( chain.PointCount(),
                                                         { arc_pos, SHAPE_IS_PT } );
...
void SHAPE_LINE_CHAIN::RemoveDuplicatePoints()
{
    ...
    std::vector<std::pair<size_t, size_t>> shapes_unique;
    ...
    while( i < PointCount() )
    {
        ...
        std::pair<size_t,size_t> shapeToKeep = m_shapes[i];
