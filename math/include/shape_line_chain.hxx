...
struct CLIPPER_Z_VALUE
{
    ...
    CLIPPER_Z_VALUE( const std::pair<size_t, size_t> aShapeIndices, size_t aOffset = 0 )
    {
        m_FirstArcIdx = aShapeIndices.first;
        m_SecondArcIdx = aShapeIndices.second;

        auto offsetVal = [&]( size_t& aVal )
                         {
                             if( aVal >= 0 )
                                 aVal += aOffset;
                         };
    ...
    size_t m_FirstArcIdx;
    size_t m_SecondArcIdx;
};
...
public:
    ...
    */
     ...
     */
    size_t ArcIndex( size_t aSegment ) const
    {
...
protected:
     ...
     */
    void splitArc(size_t aPtIndex, bool aCoincident = false );
     ...
     */
    size_t reversedArcIndex( size_t aSegment ) const
    {
        if( IsSharedPt( aSegment ) )
            return m_shapes[aSegment].first;
        else
...
private:
    ...
    static const size_t SHAPE_IS_PT;

    static const std::pair<size_t, size_t> SHAPES_ARE_PT;
